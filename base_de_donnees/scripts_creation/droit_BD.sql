...
SQL SECURITY INVOKER VIEW `vue_Ticket_visiteur` AS
SELECT
    `DB_TIX`.`Ticket`.`OBJET_TICKET` AS `OBJET_TICKET`,
    ...
    `DB_TIX`.`Ticket`.`NIV_URGENCE_DEFINITIF_TICKET` AS `NIV_URGENCE_DEFINITIF_TICKET`,
    `DB_TIX`.`Ticket`.`ETAT_TICKET` AS `ETAT_TICKET`,
    `DB_TIX`.`Ticket`.`HORODATAGE_CREATION_TICKET` AS `HORODATAGE_CREATION_TICKET`,
    `DB_TIX`.`Ticket`.`HORODATAGE_DEBUT_TRAITEMENT_TICKET` AS `HORODATAGE_DEBUT_TRAITEMENT_TICKET`,
    `DB_TIX`.`Ticket`.`HORODATAGE_RESOLUTION_TICKET` AS `HORODATAGE_RESOLUTION_TICKET`
FROM
    `DB_TIX`.`Ticket`
WHERE
    `DB_TIX`.`Ticket`.`ID_USER` = SUBSTRING_INDEX(USER(), '@', 1);
...
ALTER ALGORITHM = UNDEFINED DEFINER=`phpmyfteam`@`localhost`
 ...
 where `DB_TIX`.`Utilisateur`.`ID_USER` = substring_index(user(),'@',1)
...
`DB_TIX`.`Ticket`.`NIV_URGENCE_ESTIMER_TICKET` AS `NIV_URGENCE_ESTIMER_TICKET`
from `DB_TIX`.`Ticket`
where `DB_TIX`.`Ticket`.`ID_USER` = substring_index(user(),'@',1)
