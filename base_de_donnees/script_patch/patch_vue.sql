...
CREATE OR REPLACE VIEW vue_modif_ticket_tech AS
SELECT ID_TICKET,OBJET_TICKET, DESCRIPTION_TICKET, NIV_URGENCE_ESTIMER_TICKET, HORODATAGE_DERNIERE_MODIF_TICKET
FROM Ticket
WHERE ID_TECHNICIEN = SUBSTRING_INDEX(USER(), '@', 1)
AND ETAT_TICKET = 'Ouvert'
AND ETAT_TICKET = 'En cours de traitement';

CREATE OR REPLACE VIEW vue_modif_ticket_adm AS
SELECT ID_TICKET,OBJET_TICKET, DESCRIPTION_TICKET, NIV_URGENCE_ESTIMER_TICKET, HORODATAGE_DERNIERE_MODIF_TICKET
FROM Ticket
WHERE ID_TECHNICIEN = SUBSTRING_INDEX(USER(), '@', 1)
AND ETAT_TICKET = 'Ouvert'
